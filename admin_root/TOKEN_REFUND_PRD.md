...
**Description**: Implement server-side token refund processing for cancelled bookings.

**Specifications**:

#### Input Parameters
- `bookingIds`: Array of booking IDs to cancel
...
```javascript
// Batch read bookings from HubSpot
properties: [
  'is_active',
  'booking_status',
  'token_used',           // e.g., 'Mock Discussion Token'
  'associated_contact_id', // New property - Contact hs_object_id
  'name',
  'email'
]
```
...
- ✅ Check `token_used` property is not empty
- ✅ Check `associated_contact_id` property exists
- ❌ Skip if already cancelled (idempotent)
- ❌ Skip if no token was used (free booking)

**Step 3: Group Refunds by Token Type**
```javascript
// Group contacts by token type for batch processing
{
  'mock_discussion_token': [
    { contactId: '123', tokenCount: 1 },
    { contactId: '456', tokenCount: 1 }
  ],
  'clinical_skills_token': [
    { contactId: '789', tokenCount: 1 }
  ]
}
```

**Step 4: Batch Update Contacts**
```javascript
// For each token type, batch increment contact properties
// Use HubSpot batch update API (max 100 per request)
await hubspot.batchUpdateContacts({
  inputs: [
    {
      id: contactId,
      properties: {
...
```javascript
// Mark bookings as cancelled (existing logic)
await hubspot.batchUpdateBookings({
  inputs: bookings.map(b => ({
    id: b.id,
    properties: {
      is_active: 'Cancelled',
      token_refunded: 'true',  // NEW: Track refund status
      token_refunded_at: Date.now() // NEW: Refund timestamp
    }
  }))
});
```
...
```javascript
// Enhanced audit log with refund information
const noteContent = `
...
└─────────────────────────────────────┘
      ...
      │  └─ Get: token_used, associated_contact_id, is_active
      │
...
```json
{
  ...
  "refundTokens": true  // NEW: Optional, default true
}
```
