...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const validator = validationMiddleware('workCheckSlotBulkToggle');
    ...
    const { ids, action } = req.validatedData;
    ...
    console.error('Error in bulk toggle slots:', error);

    res.status(error.status || 500).json({
      success: false,
      error: error.message || 'Failed to toggle slot status'
    });
  }
};
