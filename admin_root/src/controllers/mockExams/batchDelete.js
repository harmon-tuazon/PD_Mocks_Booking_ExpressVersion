...
const batchDelete = async (req, res) => {
  ...
  try {
    ...
    console.log(`[BATCH-DELETE] Processing batch delete for ${sessionIds.length} sessions`);
    console.log(`[BATCH-DELETE] Admin: ${adminEmail}`);
    ...
    if (Date.now() - startTime > TIMEOUT_THRESHOLD) {
      console.warn(`[BATCH-DELETE] Timeout during validation`);
    ...
    if (results.deleted.length > 0) {
      ...
      try {
        ...
        console.log(`[BATCH-DELETE] Deleting associated bookings from Supabase for ${results.deleted.length} exams...`);
        ...
        console.log(`[BATCH-DELETE] Deleting ${results.deleted.length} exams from Supabase...`);
    ...
    if (Date.now() - startTime > TIMEOUT_THRESHOLD) {
      console.warn(`[BATCH-DELETE] Operation approaching timeout`);
    }
