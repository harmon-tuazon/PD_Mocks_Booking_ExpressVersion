...
const remove = async (req, res) => {
  try {
    ...
    if (totalBookings > 0) {
      console.log(`Deleting mock exam ${mockExamId} with ${totalBookings} total bookings (${cancelledCount} cancelled, ${activeOrCompletedBookings.length} active/completed)`);
    } else {
      console.log(`Deleting mock exam ${mockExamId} with no bookings`);
    }
    ...
    try {
      ...
      if (cancelledCount > 0) {
        console.log(`${cancelledCount} cancelled booking(s) left orphaned in Supabase`);
      }
      supabaseSynced = true;
    } catch (supabaseError) {
    ...
    console.log(`Cache invalidated for deleted mock exam ${mockExamId}`);
