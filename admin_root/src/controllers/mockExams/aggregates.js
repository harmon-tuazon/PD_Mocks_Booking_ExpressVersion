...
async function aggregates(req, res) {
  try {
    ...
    const { getExamsFromSupabase, syncExamToSupabase } = require('../../services/supabase-data');
    let aggregatesData = [];
    ...
    try {
      ...
      if (supabaseExams && supabaseExams.length > 0) {
        ...
        aggregatesData = Array.from(aggregateMap.values());
        supabaseFound = true;
        dataSource = 'supabase';
      } else {
    ...
    if (!supabaseFound) {
      ...
      aggregatesData = await hubspot.fetchMockExamsForAggregation(filters);
      dataSource = 'hubspot';
      ...
      if (aggregatesData.length > 0) {
        ...
        if (allExams.length > 0) {
          Promise.all(allExams.map(exam => syncExamToSupabase(exam))).catch(err => {
    ...
    // Sort aggregates
    const sortedAggregates = aggregatesData.sort((a, b) => {
      let comparison = 0;
      switch (sort_by) {
        case 'date':
