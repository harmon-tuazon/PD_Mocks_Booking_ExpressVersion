...
const { syncExamToSupabase, getExamsByIdsFromSupabase } = require('../../services/supabase-data');
  ...
  try {
    ...
    try {
      ...
      if (missingIds.length > 0) {
        console.log(`[BULK-UPDATE] ${missingIds.length} sessions not in Supabase, fetching from HubSpot...`);
      ...
      console.log(`[BULK-UPDATE] Fallback: Fetched ${fetchedSessions.length} sessions from HubSpot`);
    }
    ...
    for (const sessionData of processSessions) {
      ...
      if (totalBookings > 0) {
        console.log(`[BULK-UPDATE] Editing session ${sessionId} with ${totalBookings} existing booking(s)`);
      }
    ...
    console.log(`[BULK-UPDATE] Invalidating caches...`);
