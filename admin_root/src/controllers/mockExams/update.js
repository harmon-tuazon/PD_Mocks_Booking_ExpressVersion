...
const { triggerExamCascade, shouldCascadeUpdate, extractCascadeProperties } = require('../../services/supabase-webhook');
  ...
  try {
    ...
    if (updateData.exam_date && !updateData.start_time && !updateData.end_time) {
      ...
      if (currentStartTimestamp && currentEndTimestamp) {
        ...
        console.warn('[UPDATE] Current timestamps not found - cannot recalculate');
      }
    }
    ...
    try {
      ...
      if (!propertiesForSync.mock_exam_name) {
        const mockType = propertiesForSync.mock_type || currentProps.mock_type;
        const location = propertiesForSync.location || currentProps.location;
        const examDate = propertiesForSync.exam_date || currentProps.exam_date;
        propertiesForSync.mock_exam_name = `${mockType}-${location}-${examDate}`;
        console.log('[UPDATE] Generated missing mock_exam_name for Supabase sync:', propertiesForSync.mock_exam_name);
      }
    ...
    if (Object.keys(changes).length > 0) {
      ...
      hubspot.createMockExamEditNote(mockExamId, changes, user)
        .then(noteResult => {
          if (noteResult) {
            ...
            console.log(`Audit trail note creation failed for mock exam ${mockExamId}, but update was successful`);
          }
        })
        .catch(err => {
      ...
      console.log('[UPDATE] No changes detected, skipping audit trail note');
    }
    ...
    console.log(`Cache invalidated for mock exam ${mockExamId}`);
