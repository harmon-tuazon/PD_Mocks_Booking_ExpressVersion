/**
 * InstructorToggleStatusModal Component
 * Confirmation modal for bulk toggling active/inactive status of instructors
 *
 * Features:
 * - Shows breakdown of instructors to be activated/deactivated
...
const InstructorToggleStatusModal = ({
  isOpen,
  onClose,
  onConfirm,
  selectedInstructors,
  ...
  // Calculate instructor breakdown
  const instructorBreakdown = useMemo(() => {
    ...
    // Count instructors that will be activated (inactive -> active)
    // is_active is a boolean in the instructors table
    const toActivate = selectedInstructors.filter(instructor =>
      instructor.is_active === false
    ).length;

    // Count instructors that will be deactivated (active -> inactive)
    const toDeactivate = selectedInstructors.filter(instructor =>
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md sm:p-6">
                  ...
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    ...
                    <div className="mt-4">
                      <p className="text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span className="font-semibold">{instructorBreakdown.total} instructor{instructorBreakdown.total !== 1 ? 's' : ''}</span>:
                      </p>

                      {instructorBreakdown.total > 0 && (
                        <div className="mt-3 space-y-2">
                          {instructorBreakdown.toActivate > 0 && (
                            <div className="flex items-center text-sm">
                              <span className="text-green-600 dark:text-green-400 mr-2">•</span>
                              <span className="text-gray-700 dark:text-gray-300">
                                {instructorBreakdown.toActivate} instructor{instructorBreakdown.toActivate !== 1 ? 's' : ''} will be activated
                              </span>
                            </div>
                          )}

                          {instructorBreakdown.toDeactivate > 0 && (
                            <div className="flex items-center text-sm">
                              <span className="text-red-600 dark:text-red-400 mr-2">•</span>
                              <span className="text-gray-700 dark:text-gray-300">
                                {instructorBreakdown.toDeactivate} instructor{instructorBreakdown.toDeactivate !== 1 ? 's' : ''} will be deactivated
                              </span>
                            </div>
                          )}
                        </div>
                ...
                <div className="mt-6 sm:mt-4 sm:flex sm:flex-row-reverse sm:ml-10">
                  <button
                    ...
                    disabled={isSubmitting || instructorBreakdown.total === 0}
                  >
                    {isSubmitting ? (
                      <>
...
export default InstructorToggleStatusModal;
