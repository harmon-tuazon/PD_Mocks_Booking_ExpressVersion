/**
 * GroupToggleStatusModal Component
 * Confirmation modal for bulk toggling active/inactive status of groups
 *
 * Features:
 * - Shows breakdown of groups to be activated/deactivated
...
const GroupToggleStatusModal = ({
  ...
  const groupBreakdown = useMemo(() => {
    if (!selectedGroups || selectedGroups.length === 0) {
      return { toActivate: 0, toDeactivate: 0, toComplete: 0, total: 0 };
    }

    // Count groups that will be activated (inactive -> active)
    const toActivate = selectedGroups.filter(group =>
      group.status === 'inactive'
    ).length;

    // Count groups that will be deactivated (active -> inactive)
    const toDeactivate = selectedGroups.filter(group =>
      group.status === 'active'
    ).length;

    // Count groups that are completed (will toggle to inactive)
    const toComplete = selectedGroups.filter(group =>
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md sm:p-6">
                  ...
                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                    ...
                    <div className="mt-4">
                      <p className="text-sm text-gray-700 dark:text-gray-300">
                        You are about to toggle the status for{' '}
                        <span className="font-semibold">{groupBreakdown.total} group{groupBreakdown.total !== 1 ? 's' : ''}</span>:
                      </p>

                      {groupBreakdown.total > 0 && (
                        <div className="mt-3 space-y-2">
                          {groupBreakdown.toActivate > 0 && (
                            <div className="flex items-center text-sm">
                              <span className="text-green-600 dark:text-green-400 mr-2">•</span>
                              <span className="text-gray-700 dark:text-gray-300">
                                {groupBreakdown.toActivate} group{groupBreakdown.toActivate !== 1 ? 's' : ''} will be activated
                              </span>
                            </div>
                          )}

                          {groupBreakdown.toDeactivate > 0 && (
                            <div className="flex items-center text-sm">
                              <span className="text-red-600 dark:text-red-400 mr-2">•</span>
                              <span className="text-gray-700 dark:text-gray-300">
                                {groupBreakdown.toDeactivate} group{groupBreakdown.toDeactivate !== 1 ? 's' : ''} will be deactivated
                              </span>
                            </div>
                          )}

                          {groupBreakdown.toComplete > 0 && (
                            <div className="flex items-center text-sm">
                              <span className="text-blue-600 dark:text-blue-400 mr-2">•</span>
                              <span className="text-gray-700 dark:text-gray-300">
                                {groupBreakdown.toComplete} completed group{groupBreakdown.toComplete !== 1 ? 's' : ''} will be deactivated
                              </span>
                            </div>
                          )}
                        </div>
                ...
                <div className="mt-6 sm:mt-4 sm:flex sm:flex-row-reverse sm:ml-10">
                  <button
                    ...
                    disabled={isSubmitting || groupBreakdown.total === 0}
                  >
                    {isSubmitting ? (
                      <>
...
export default GroupToggleStatusModal;
