...
const TEST_STUDENT_ID = process.env.TEST_STUDENT_ID || '1599999';
const TEST_EMAIL = process.env.TEST_EMAIL || 'htuazon@prepdoctors.com';
...
async function runLoadTest() {
  ...
  if (otherErrors.length > 0) {
    ...
    Object.keys(errorsByStatus).forEach(status => {
      const count = errorsByStatus[status].length;
      console.log(`      - Status ${status}: ${count} error(s)`);
    });
  }
  ...
  // Show other error details (non-429 errors)
  if (otherErrors.length > 0) {
    console.log(`\n⚠️ Other Error Details:`);

    // Group by error message for cleaner output
    const errorMessages = {};
    otherErrors.forEach(r => {
      ...
      errorMessages[errorKey].push(r.requestNum);
    });

    Object.keys(errorMessages).forEach(errorKey => {
      const requests = errorMessages[errorKey];
      console.log(`\n   ${errorKey}`);
      console.log(`   Affected requests: #${requests.join(', #')}`);
    });
  }
  ...
  if (insufficientCreditsRequests.length > 0) {
    ...
    const examples = insufficientCreditsRequests.slice(0, 3);
    examples.forEach(r => {
      console.log(`   - Request #${r.requestNum} (${r.mockType}): 0 credits`);
    });
    if (insufficientCreditsRequests.length > 3) {
      console.log(`   ... and ${insufficientCreditsRequests.length - 3} more`);
    }
  }
