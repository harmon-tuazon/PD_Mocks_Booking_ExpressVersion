...
exports.main = async (event, callback) => {
    ...
    try {
        ...
        // Fetch the display name for the user who initiated the update using hs_searchable_calculated_name
        let lastUpdatedBy = 'Admin';
        const updatedByUserId = event.inputFields[UPDATED_BY];

        if (updatedByUserId) {
            try {
                // Fetch user details using the user ID and get hs_searchable_calculated_name property
                const userResponse = await hubspotApiCall('get', `/crm/v3/objects/users/${updatedByUserId}?properties=hs_searchable_calculated_name`);
                lastUpdatedBy = userResponse.properties?.hs_searchable_calculated_name || `User ${updatedByUserId}`;
                console.log(`✅ Fetched user name: ${lastUpdatedBy} (ID: ${updatedByUserId})`);
            } catch (userError) {
                console.warn(`⚠️ Could not fetch user details for ID ${updatedByUserId}, using default name.`, userError.message);
                lastUpdatedBy = 'Admin';
            }
        }
