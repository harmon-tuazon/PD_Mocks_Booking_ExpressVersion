...
try {
  ...
  console.warn('⚠️ Redis not available for work-checks/[id]:', error.message);
}
...
module.exports = async (req, res) => {
  if (req.method !== 'DELETE') {
    return res.status(405).json({
      success: false,
      error: { code: 'METHOD_NOT_ALLOWED', message: 'Only DELETE requests are allowed' }
    });
  }
  ...
  try {
    ...
    const { error, value } = schemas.workCheckCancel.validate(req.body);
    if (error) {
      return res.status(400).json({
        success: false,
    ...
    const { student_id, email, reason } = value;

    console.log(`🗑️ [WORK-CHECK] Cancelling booking: ${bookingId} for ${student_id}`);
